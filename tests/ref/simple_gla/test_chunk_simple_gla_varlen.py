...
_VARLEN_SHAPES = [
  ...
  dict(T=128, H=2, K=32, V=64, cu_seqlens=[0, 32, 128], C=64, seed=7),
...
@pytest.mark.parametrize(
  "cfg", _VARLEN_SHAPES, ids=[_varlen_id(c) for c in _VARLEN_SHAPES]
)
def test_simple_gla_varlen_fwd(cfg, gate):
  ...
  assert compare_tensor(
    "output", o_var, o_ind, atol=1e-5, rtol=1e-5, compare_dtype=np.float64
  )
...
def test_simple_gla_varlen_with_h0():
  ...
  assert compare_tensor(
    "final_state", ht_var, ht_ind, atol=1e-5, rtol=1e-5, compare_dtype=np.float64
  )
...
def test_simple_gla_varlen_bwd(cfg, gate):
  ...
  g = (
    jax.nn.log_sigmoid(jax.random.normal(keys[3], (1, T, H), dtype=jnp.float64))
    if gate == "g"
    else None
  )
  g_gamma = (
    -jax.nn.softplus(jax.random.normal(keys[4], (H,), dtype=jnp.float64))
    if gate == "g_gamma"
    else None
  )
  ...
  assert compare_tensor(
    "dq", ref_dq, dq, atol=1e-8, rtol=1e-8, compare_dtype=np.float64
  )
  assert compare_tensor(
    "dk", ref_dk, dk, atol=1e-8, rtol=1e-8, compare_dtype=np.float64
  )
  assert compare_tensor(
    "dv", ref_dv, dv, atol=1e-8, rtol=1e-8, compare_dtype=np.float64
  )
  if g is not None:
    assert compare_tensor(
      "dg", ref_dg, dg, atol=1e-8, rtol=1e-8, compare_dtype=np.float64
    )
  else:
    assert dg is None
...
@pytest.mark.parametrize(
  "cfg", GPU_VARLEN_CASES, ids=[_gpu_case_id(c) for c in GPU_VARLEN_CASES]
)
def test_simple_gla_varlen_cpu_vs_triton(cfg):
  ...
  assert compare_tensor(
    "output",
    o_tri.float().numpy(),
    np.array(o_cpu, dtype=np.float32),
    atol=atol,
    rtol=rtol,
  )
  ...
  if ht_tri is not None and ht_cpu is not None:
    assert compare_tensor(
      "final_state",
      ht_tri.float().numpy(),
      np.array(ht_cpu, dtype=np.float32),
      atol=atol,
      rtol=rtol,
    )
