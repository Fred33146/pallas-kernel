"""simple_gla forward + backward: JAX naive vs FLA Triton GPU."""
...
# ============================================================================
# Backward: Triton fused_recurrent (gold via autograd) vs JAX naive autodiff
# ============================================================================
...
def _run_naive_bwd(q, k, v, do, *, g=None, g_gamma=None, h0=None, scale=None):
    ...
    g_jax = _torch_to_jax(g) if g is not None else None
    g_gamma_jax = _torch_to_jax(g_gamma) if g_gamma is not None else None
    h0_jax = _torch_to_jax(h0) if h0 is not None else None
...
def test_triton_vs_naive_bwd(cfg):
    """Triton fused_recurrent backward (gold) vs JAX naive autodiff."""
    B, T, H, K, V = cfg["B"], cfg["T"], cfg["H"], cfg["K"], cfg["V"]
    atol = cfg.get("atol", 1e-4)
