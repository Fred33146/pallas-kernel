...
def test_fused_recurrent_vs_chunk_fp64():
    ...
    q, k, v, g, _, h0 = _make_inputs(B, T, H, K, V, jnp.float64, seed=42, gate="g", h0=True)
...
def test_fused_recurrent_cu_seqlens_fp64_g():
    ...
    o1, s1 = fused_recurrent_simple_gla(
        q[:, :T1], k[:, :T1], v[:, :T1], g=g[:, :T1],
        initial_state=h0[0:1], output_final_state=True,
    )
    o2, s2 = fused_recurrent_simple_gla(
        q[:, T1:], k[:, T1:], v[:, T1:], g=g[:, T1:],
        initial_state=h0[1:2], output_final_state=True,
    )
...
def test_fused_recurrent_cu_seqlens_fp64_g_gamma():
    ...
    o1, s1 = fused_recurrent_simple_gla(
        q[:, :T1], k[:, :T1], v[:, :T1], g_gamma=g_gamma,
        initial_state=h0[0:1], output_final_state=True,
    )
    o2, s2 = fused_recurrent_simple_gla(
        q[:, T1:], k[:, T1:], v[:, T1:], g_gamma=g_gamma,
...
def _triton_case_id(c):
    parts = [f"B{c['B']}_T{c['T']}_H{c['H']}_K{c['K']}_V{c['V']}_{c['dtype']}_{c['gate']}"]
    if c.get("h0"):
        parts.append("h0")
    return "-".join(parts)
...
def test_cpu_vs_triton_fwd(cfg):
    ...
    o_cpu, s_cpu = fused_recurrent_simple_gla(
        q_j, k_j, v_j, g=g_j, g_gamma=g_gamma_j,
        initial_state=h0_j, output_final_state=True,
    )
